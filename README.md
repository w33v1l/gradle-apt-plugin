...
* automatically configures the corresponding `JavaCompile`, `GroovyCompile` and `ScalaCompile` tasks to make use of these configurations, when the `java`,  `groovy` or `scala` plugin is applied;
...
The `*Only` configurations are part of the `classpath` of the `JavaCompile`, `GroovyCompile` and `ScalaCompile` tasks, whereas the `apt` and `testApt` configurations are turned into `-processorpath` compiler arguments.
...
Finally, note that those configurations don't extend each others: `testCompileOnly` doesn't extend `compileOnly`, and `testApt` doesn't extend `apt`; those configurations are only use for their respective `JavaCompile`, `GroovyCompile` and `ScalaCompile` tasks.
...
Each `JavaCompile`, `GroovyCompile` and `ScalaCompile` task gains a couple properties:
...
For each source set, the corresponding `JavaCompile`, `GroovyCompile` and `ScalaCompile` tasks are configured such that:
